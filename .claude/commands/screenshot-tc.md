...
3. Voeg `page.screenshot(path=...)` toe op dat moment. Sla de screenshot op in `uitwerking/deelopdracht-7-testrapportage/screenshots/` met bestandsnaam `<TC-ID>-<status>.png` (bijv. `tc001-geslaagd.png`).
4. Zorg dat de map bestaat door `os.makedirs(..., exist_ok=True)` toe te voegen indien dat nog niet in het script staat.
5. Voeg het pad naar de screenshot toe aan het `result()`-aanroep als extra notitie, zodat het in de rapportage terechtkomt.
